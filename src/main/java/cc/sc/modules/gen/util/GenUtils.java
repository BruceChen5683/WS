 ...
 */
package cc.sc.modules.gen.util;
...
import cc.sc.common.mapper.JaxbMapper;
import cc.sc.common.utils.DateUtils;
import cc.sc.common.utils.FileUtils;
import cc.sc.common.utils.FreeMarkers;
import cc.sc.common.utils.StringUtils;
import cc.sc.modules.gen.entity.GenCategory;
import cc.sc.modules.gen.entity.GenConfig;
import cc.sc.modules.gen.entity.GenScheme;
import cc.sc.modules.gen.entity.GenTable;
...
public class GenUtils {
	...
	public static void initColumnField(GenTable genTable) {
		for (GenTableColumn column : genTable.getColumnList()) {
			...
			// 编辑字段
			if (!StringUtils.equalsIgnoreCase(column.getName(), "id") && !StringUtils.equalsIgnoreCase(column.getName(), "create_by") && !StringUtils.equalsIgnoreCase(column.getName(), "create_date")
					&& !StringUtils.equalsIgnoreCase(column.getName(), "del_flag")) {
				column.setIsEdit("1");
			}

			// 列表字段
			if (StringUtils.equalsIgnoreCase(column.getName(), "name") || StringUtils.equalsIgnoreCase(column.getName(), "title") || StringUtils.equalsIgnoreCase(column.getName(), "remarks")
		...
		try {
			...
			if (file != null) {
				return file.getAbsolutePath() + File.separator
						+ StringUtils.replaceEach(GenUtils.class.getName(), new String[] { "util." + GenUtils.class.getSimpleName(), "." }, new String[] { "template", File.separator });
			}
		} catch (Exception e) {
			logger.error("{}", e);
		}
	...
	public static Map<String, Object> getDataModel(GenScheme genScheme) {
		...
		model.put("urlPrefix",
				model.get("moduleName") + (StringUtils.isNotBlank(genScheme.getSubModuleName()) ? "/" + StringUtils.lowerCase(genScheme.getSubModuleName()) : "") + "/" + model.get("className"));
