...
@property (nonatomic, assign) NSInteger defaultV;
...
+(instancetype)showWithDataArray:(NSArray *)array block:(selectedBlock)block bottomDistance:(CGFloat)bottomDistance defaultV:(NSInteger)defaultV{
    SinglePickerView *pick = [[SinglePickerView alloc] init];
    pick.bottomDistance = bottomDistance;
    pick.defaultV = defaultV;
...
- (void)show {
    ...
    for (NSInteger i= 0; i < self.dataArray.count; i++) {
       if ([self.dataArray[i] integerValue] == self.defaultV) {
