...
@property (nonatomic, strong) UILabel *pTitleLbl;
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.barView addSubview:self.pTitleLbl];
...
- (void)show {
    ...
    contentViewFrame.origin.y -= self.contentView.frame.size.height + self.bottomDistance;
...
- (UILabel *)pTitleLbl {
    if (!_pTitleLbl) {
        _pTitleLbl = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, kWindowW,self.barViewHeight)];
        _pTitleLbl.backgroundColor = [UIColor redColor];
        _pTitleLbl.text = @"商家年入驻费用";
        _pTitleLbl.font = [UIFont systemFontOfSize:17];
        _pTitleLbl.textColor = [UIColor whiteColor];
        _pTitleLbl.textAlignment = NSTextAlignmentCenter;
    }
    return _pTitleLbl;
}
